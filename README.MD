...
MemoMindAI는 업무 메모를 구조화하여 로컬 LLM에 자연어로 질문할 수 있는 업무지식·인수인계 도구입니다.

사용자가 등록한 메모는 JSON 형식으로 저장되며, 저장된 메모는 RAG(검색증강)으로 답변합니다.
...
프로젝트 루트에서 가상환경을 활성화한 뒤 운영체제에 맞는 명령을 실행합니다.
